...
var data = fs.readFileSync('data/orgs.tsv','utf8')
      ...
      if(reposCount < reposNumber){
        ...
        if(repos.length){
          repos.forEach(function(d){
            fs.writeFileSync('output/orgs/' + org + '_' + d.name + '.json', JSON.stringify(d), 'utf8');
            var elm = {
              user: org,
              name: d.name,
              owner: d.owner.login,
